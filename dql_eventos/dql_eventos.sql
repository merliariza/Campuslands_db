...
JOIN 
    camper_groups cg ON c.id = cg.camper_id
JOIN 
...
JOIN 
    routes r ON tg.route_id = r.id
JOIN 
    trainers t ON tg.trainer_id = t.id
JOIN 
...
JOIN 
    persons p_camper ON c.person_id = p_camper.id
JOIN 
    training_groups tg ON cg.group_id = tg.id
JOIN 
    routes r ON tg.route_id = r.id
JOIN 
    trainers t ON tg.trainer_id = t.id
JOIN 
    persons p_trainer ON t.person_id = p_trainer.id;
...
SELECT 
    ...
    tb.end_time
FROM 
    trainer_schedules ts
JOIN 
    trainers t ON ts.trainer_id = t.id
JOIN 
    persons p ON t.person_id = p.id
JOIN 
    time_blocks tb ON ts.time_block_id = tb.id
JOIN 
...
HAVING 
    (COUNT(cg.camper_id) / cl.capacity * 100) > 80;
...
JOIN 
    modules m ON me.module_id = m.id
WHERE 
    me.practice_score > me.theory_score;
...
JOIN 
    routes r ON tg.route_id = r.id
JOIN 
    route_sgdb rs ON r.id = rs.route_id
JOIN 
    sgdb s ON rs.database_id = s.id
WHERE 
